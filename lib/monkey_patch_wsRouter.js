...
patch.addListener = function(data, connection) {
  ...
  this.listeners[token] = this.listeners[token] || {meta:[], io:[], master:{}, info:{}};
  this.listeners[token][channel].push(connection);
  return this;
}

patch.deleteListener = function(data, connection) {
  var token = data.token;
  var channel = data.channel;

  if (!token) throw new Error("token shouldn't be empty.");

  if (this.listeners[token]) {
    if (channel) {
      var idx = this.listeners[token][channel].indexOf(connection);
      this.listeners[token][channel].splice(idx,1);
    } else {
      var idx = this.listeners[token]['io'].indexOf(connection);
      this.listeners[token]['io'].splice(idx,1);
      idx = this.listeners[token]['meta'].indexOf(connection);
      this.listeners[token]['meta'].splice(idx,1);
    }
  }
  return this;
}
...
patch.addMaster = function(data, connection) {
  ...
  this.listeners[token].info.size = this.listeners[token].info.size || size;
  return this;
}
